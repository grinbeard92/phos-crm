#!/bin/bash
# Create custom business objects for Phos Industries CRM

API_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..TDPdX88kBxuUXnGwieAbt6Naod3XLtDDEhIdFmd7NeE"
METADATA_ENDPOINT="http://localhost:3000/metadata"

echo "=== Creating Phos Industries Custom Business Objects ==="
echo ""

# 1. Create Projects Object
echo "1. Creating Projects object..."
PROJECTS_RESPONSE=$(curl -s -X POST "$METADATA_ENDPOINT" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{
    "query": "mutation CreateObject($input: CreateObjectInput!) { createOneObject(input: $input) { id nameSingular namePlural labelSingular labelPlural } }",
    "variables": {
      "input": {
        "object": {
          "nameSingular": "project",
          "namePlural": "projects",
          "labelSingular": "Project",
          "labelPlural": "Projects",
          "description": "Project management with Gantt and Kanban support",
          "icon": "IconBriefcase"
        }
      }
    }
  }')
echo "$PROJECTS_RESPONSE" | jq '.'
echo ""

# 2. Create Expenses Object
echo "2. Creating Expenses object..."
EXPENSES_RESPONSE=$(curl -s -X POST "$METADATA_ENDPOINT" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{
    "query": "mutation CreateObject($input: CreateObjectInput!) { createOneObject(input: $input) { id nameSingular namePlural labelSingular labelPlural } }",
    "variables": {
      "input": {
        "object": {
          "nameSingular": "expense",
          "namePlural": "expenses",
          "labelSingular": "Expense",
          "labelPlural": "Expenses",
          "description": "Expense tracking with receipts and project association",
          "icon": "IconReceipt"
        }
      }
    }
  }')
echo "$EXPENSES_RESPONSE" | jq '.'
echo ""

# 3. Create Quotes Object
echo "3. Creating Quotes object..."
QUOTES_RESPONSE=$(curl -s -X POST "$METADATA_ENDPOINT" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{
    "query": "mutation CreateObject($input: CreateObjectInput!) { createOneObject(input: $input) { id nameSingular namePlural labelSingular labelPlural } }",
    "variables": {
      "input": {
        "object": {
          "nameSingular": "quote",
          "namePlural": "quotes",
          "labelSingular": "Quote",
          "labelPlural": "Quotes",
          "description": "Customer quotes and proposals",
          "icon": "IconFileText"
        }
      }
    }
  }')
echo "$QUOTES_RESPONSE" | jq '.'
echo ""

# 4. Create Invoices Object
echo "4. Creating Invoices object..."
INVOICES_RESPONSE=$(curl -s -X POST "$METADATA_ENDPOINT" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{
    "query": "mutation CreateObject($input: CreateObjectInput!) { createOneObject(input: $input) { id nameSingular namePlural labelSingular labelPlural } }",
    "variables": {
      "input": {
        "object": {
          "nameSingular": "invoice",
          "namePlural": "invoices",
          "labelSingular": "Invoice",
          "labelPlural": "Invoices",
          "description": "Billing and invoices with Stripe integration",
          "icon": "IconCurrencyDollar"
        }
      }
    }
  }')
echo "$INVOICES_RESPONSE" | jq '.'
echo ""

# 5. Create Support Tickets Object
echo "5. Creating Support Tickets object..."
TICKETS_RESPONSE=$(curl -s -X POST "$METADATA_ENDPOINT" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $API_KEY" \
  -d '{
    "query": "mutation CreateObject($input: CreateObjectInput!) { createOneObject(input: $input) { id nameSingular namePlural labelSingular labelPlural } }",
    "variables": {
      "input": {
        "object": {
          "nameSingular": "supportTicket",
          "namePlural": "supportTickets",
          "labelSingular": "Support Ticket",
          "labelPlural": "Support Tickets",
          "description": "Customer support ticket tracking",
          "icon": "IconHelp"
        }
      }
    }
  }')
echo "$TICKETS_RESPONSE" | jq '.'
echo ""

echo "✓ Custom object creation complete!"
echo "Check http://localhost:3001/settings/objects for your new objects"
